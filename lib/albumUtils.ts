...
export async function createAlbumPage(imageData: Record<string, string>): Promise<string> {
    ...
    ctx.fillStyle = '#fafaf5'; // A warm, stone-like color
    ...
    ctx.fillText('💫 Re:You', canvasWidth / 2, 150);

    ctx.font = `50px 'Inter', sans-serif`;
    ctx.fillStyle = '#57534e'; // stone-600
    ctx.fillText('A message from your past.', canvasWidth / 2, 220);
    ...
    reversedImages.forEach(({ decade, img }, reversedIndex) => {
        ...
        ctx.font = `70px 'Playfair Display', serif`;
        ctx.textAlign = 'center';
